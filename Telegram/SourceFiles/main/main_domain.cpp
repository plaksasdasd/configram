...
not_null<Main::Account*> Domain::addWithAuthorization(
		MTP::Environment environment,
		QByteArray mtpAuthorization,
		QByteArray sessionSettings) {
	Expects(started());
	Expects(_accounts.size() < kPremiumMaxAccounts);
	Expects(!mtpAuthorization.isEmpty());
	...
	const auto account = _accounts.back().account.get();
	account->setMtpAuthorization(std::move(mtpAuthorization));
	if (!sessionSettings.isEmpty()) {
		auto settings = std::make_unique<Main::SessionSettings>();
		settings->addFromSerialized(sessionSettings);
...
void Domain::addActivatedWithAuthorization(
		MTP::Environment environment,
