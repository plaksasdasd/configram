...
void ImportAuthConfig(not_null<Window::SessionController*> controller) {
	...
	FileDialog::GetOpenPath(
		...
		[=](FileDialog::OpenResult &&result) {
			...
			if (data.isEmpty()) {
				weak->showToast(tr::lng_auth_config_error_read(tr::now));
				return;
			}
			const auto parsed = Main::ParseAuthConfig(std::move(data));
			if (!parsed) {
				weak->showToast(tr::lng_auth_config_error_invalid(tr::now));
				return;
			}
			auto &domain = weak->session().domain();
			const auto userId = UserId(parsed->userId);
			for (const auto &[index, account] : domain.accounts()) {
				if (account->sessionExists()
					&& account->mtp().environment() == parsed->environment
					&& account->session().userId() == userId) {
					domain.maybeActivate(account.get());
					weak->showToast(tr::lng_auth_config_error_exists(tr::now));
					return;
				}
			}
			if (domain.accounts().size() >= domain.maxAccounts()) {
				weak->show(Box(AccountsLimitBox, &weak->session()));
				return;
			}
			const auto prepared = *parsed;
			weak->window().preventOrInvoke([=] {
				if (!prepared.appSettings.isEmpty()) {
					Core::App().settings().addFromSerialized(prepared.appSettings);
					Core::App().saveSettingsDelayed();
				}
				weak->session().domain().addActivatedWithAuthorization(
...
private:
	...
	[[nodiscard]] not_null<Ui::SlideWrap<Ui::SettingsButton>*> setupAuthByConfig();
	void rebuild();
	...
	Ui::SlideWrap<Ui::SettingsButton> *_authByConfig = nullptr;
	base::flat_map<
...
void AccountsList::setup() {
	...
	_authByConfig = setupAuthByConfig();
...
not_null<Ui::SlideWrap<Ui::SettingsButton>*> AccountsList::setupAuthByConfig() {
	const auto result = _outer->add(
		object_ptr<Ui::SlideWrap<Ui::SettingsButton>>(
			_outer.get(),
			CreateButtonWithIcon(
