...
[[nodiscard]] QString SettingsConfigFileFilter() {
	return u"Telegram Config (*.tdesktop-config);;"_q
		+ FileDialog::AllFilesFilter();
}

void SaveSettingsConfig(not_null<Window::SessionController*> controller) {
	const auto weak = base::make_weak(controller);
	FileDialog::GetWritePath(
		Core::App().getFileDialogParent(),
		tr::lng_settings_config_save_title(tr::now),
		SettingsConfigFileFilter(),
		QString(),
		[=](QString &&path) {
			if (path.isEmpty() || !weak) {
				return;
			}
			const auto account = &weak->session().account();
			if (!account->sessionExists()) {
				weak->showToast(tr::lng_settings_config_error_write(tr::now));
				return;
			}
			const auto data = Main::SerializeAuthConfig(
				account->mtp().environment(),
				account->serializeMtpAuthorization(),
				weak->session().settings().serialize(),
				Core::App().settings().serialize());
			...
			weak->showToast(tr::lng_settings_config_saved(tr::now));
		});
}

void ApplySettingsConfig(not_null<Window::SessionController*> controller) {
	const auto weak = base::make_weak(controller);
	FileDialog::GetOpenPath(
		Core::App().getFileDialogParent(),
		tr::lng_settings_config_open_title(tr::now),
		SettingsConfigFileFilter(),
		[=](FileDialog::OpenResult &&result) {
			...
			if (!result.paths.isEmpty()) {
				...
				if (!file.open(QIODevice::ReadOnly)) {
					weak->showToast(
						tr::lng_settings_config_error_read(tr::now));
					return;
				}
				data = file.readAll();
				file.close();
			...
			if (data.isEmpty()) {
				weak->showToast(tr::lng_settings_config_error_read(tr::now));
				return;
			}
			const auto parsed = Main::ParseAuthConfig(std::move(data));
			if (!parsed) {
				weak->showToast(
					tr::lng_settings_config_error_invalid(tr::now));
				return;
			}
			if (!parsed->appSettings.isEmpty()) {
				Core::App().settings().addFromSerialized(parsed->appSettings);
			}
			if (!parsed->sessionSettings.isEmpty()) {
				weak->session().settings().addFromSerialized(parsed->sessionSettings);
			}
			Core::App().saveSettingsDelayed();
			weak->session().saveSettingsDelayed();
			weak->showToast(tr::lng_settings_config_applied(tr::now));
		});
}

void ShowSettingsConfigBox(
		not_null<Window::SessionController*> controller) {
	controller->show(Box([=](not_null<Ui::GenericBox*> box) {
		box->setTitle(tr::lng_settings_config_title());
		box->setWidth(st::boxWideWidth);

		const auto content = box->verticalLayout();
		const auto createId = u"create"_q;
		const auto applyId = u"apply"_q;
		const auto tabs = content->add(
			object_ptr<Ui::SubTabs>(
				...
				std::vector<Ui::SubTabs::Tab>{
					{
						createId,
						tr::lng_settings_config_tab_create(
							tr::now,
							Ui::Text::WithEntities),
					},
					{
						applyId,
						tr::lng_settings_config_tab_apply(
							tr::now,
							Ui::Text::WithEntities),
					},
				}),
		...
		const auto createWrap = content->add(
			object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
				content,
				object_ptr<Ui::VerticalLayout>(content)));
		const auto create = createWrap->entity();
		create->add(
			object_ptr<Ui::FlatLabel>(
				create,
				tr::lng_settings_config_create_about(),
				st::boxLabel),
			st::boxRowPadding);

		const auto applyWrap = content->add(
			object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
				content,
				object_ptr<Ui::VerticalLayout>(content)));
		const auto apply = applyWrap->entity();
		apply->add(
			object_ptr<Ui::FlatLabel>(
				apply,
				tr::lng_settings_config_apply_about(),
				st::boxLabel),
			st::boxRowPadding);

		const auto tabValue = state->tab.value();
		createWrap->toggleOn(tabValue | rpl::map([=](const QString &id) {
			return (id == createId);
		}));
		applyWrap->toggleOn(tabValue | rpl::map([=](const QString &id) {
			return (id == applyId);
		}));

		const auto createButton = box->addButton(
			tr::lng_settings_config_create_button(),
			[=] { SaveSettingsConfig(controller); });
		const auto applyButton = box->addButton(
			tr::lng_settings_config_apply_button(),
			[=] { ApplySettingsConfig(controller); });
		box->addButton(tr::lng_cancel(), [=] { box->closeBox(); });

		createButton->showOn(tabValue | rpl::map([=](const QString &id) {
			return (id == createId);
		}));
		applyButton->showOn(tabValue | rpl::map([=](const QString &id) {
			return (id == applyId);
		}));
	}));
}
...
void SetupConfigButton(
	...
	const auto button = AddButtonWithIcon(
		container,
		tr::lng_settings_config_button(),
		st::settingsButton,
		{ &st::menuIconCustomize });
	button->setClickedCallback([=] {
		ShowSettingsConfigBox(controller);
	});
}
...
void SetupSections(
	...
	SetupConfigButton(controller, container);
